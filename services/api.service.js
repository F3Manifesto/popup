...
import {
  ...
  MAINNET_CHAINID,
  POLYGON_CHAINID,
  GRAPH_API_URL_DIGITALAX_POLYGON,
} from "../constants";
...
export const getPayableTokenReport = async (id) => {
  return request(GRAPH_API_URL_DIGITALAX_POLYGON, PAYABLE_TOKEN_REQUEST, {
    id,
  });
};

export const getMarketplacePurchaseHistories = async (orderId) => {
  return request(GRAPH_API_URL_POLYGON, DRIP_MARKETPLACE_PURCHASE_HISTORES, {
    orderId,
  });
};

export const getDigitalaxGarmentV2CollectionById = async (id) => {
  return request(GRAPH_API_URL_POLYGON, DIGITALAX_GARMENT_V2_COLLECTION_BY_ID, {
    id,
  });
};
...
const apiRequest = (chainId, gql, params) =>
  request(getAPIUrlByChainId(chainId), gql, params);
...
export const getDigitalaxNFTStakersByGarments = async (
  ...
  apiRequest(chainId, DIGITALAX_NFT_STAKERS_BY_GARMENTS, {
    garmentIDs,
    first,
    lastID,
  });
...
export const getGuildWhitelistedNFTStakersByGarments = async (
  ...
  apiRequest(chainId, GUILD_WHITELISTED_NFT_STAKERS_BY_GARMENTS, {
    garmentIDs,
    first,
    lastID,
  });
...
export const getGuildWhitelistedNFTStakersByStaker = async (
  ...
  apiRequest(chainId, GUILD_WHITELISTED_NFT_STAKERS_BY_STAKER, {
    staker,
    first,
    lastID,
  });

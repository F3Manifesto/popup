export const filterProducts = (
  ...
  if (sortBy) {
    switch (sortBy) {
      ...
      case "4":
        filteredProducts.sort((a, b) => {
          const aAmount = a.amountSold || 0;
          const bAmount = b.amountSold || 0;
      ...
      case "5":
        filteredProducts = isCollection
          ? filteredProducts.filter((prod) =>
              prod?.products.find((item: any) => item?.rarity === "Exclusive")
            )
      ...
      case "6":
        filteredProducts = isCollection
          ? filteredProducts.filter((prod) =>
              prod?.products.find((item: any) => item?.rarity === "Semi-Rare")
            )
      ...
      case "7":
        filteredProducts = isCollection
          ? filteredProducts.filter((prod) =>
              prod?.products.find((item: any) => item?.rarity === "Common")
            )
